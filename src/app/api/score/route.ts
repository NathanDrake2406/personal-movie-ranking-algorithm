...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!movie.imdbId) {
      return NextResponse.json(
        { error: "Could not determine IMDb ID" },
        { status: 422 },
      );
    }
    ...
    after(async () => {
      try {
        ...
        log.warn("score_deferred_failed", {
          tmdbId,
          error: (deferredErr as Error).message,
        });
      }
    });
    ...
    if (isAbortError(err)) {
      log.info("score_request_aborted", { tmdbId });
      return new Response(null, { status: 499 });
    }
    log.error("score_request_failed", {
      tmdbId,
