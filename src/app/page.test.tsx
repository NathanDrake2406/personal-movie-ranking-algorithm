...
async function selectMovieWithPayload(moviePayload: Record<string, unknown>) {
    ...
    if (url.includes("/api/search")) {
      return Promise.resolve({
        ...
        json: () =>
          Promise.resolve({
            results: [
              { id: 1, title: "Test Movie", year: "2024", poster: null },
            ],
          }),
      });
    }
...
describe("Home page", () => {
  ...
  describe("CreditsList rendering", () => {
    it("renders all credit types when all fields are present", async () => {
      ...
      expect(
        screen.getByText("Timothée Chalamet, Zendaya"),
      ).toBeInTheDocument();
