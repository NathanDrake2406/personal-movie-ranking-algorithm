...
describe("ThemesSection", () => {
  ...
  it("renders nothing when themes is empty", () => {
    const { container } = render(
      <ThemesSection themes={[]} imdbId="tt1234567" />,
    );
  ...
  it("renders label and all chips", () => {
    ...
    expect(
      screen.getByText("What resonated with audiences"),
    ).toBeInTheDocument();
  ...
  it("clicking chip fetches summary and displays it", async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          summary: "The acting performances were outstanding.",
        }),
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("The acting performances were outstanding."),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows error on fetch failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Summary unavailable. Try the IMDb link below."),
      ).toBeInTheDocument();
    });
  });
