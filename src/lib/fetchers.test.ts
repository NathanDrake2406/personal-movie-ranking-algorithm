...
const baseCtx = {
  ...
  wikidata: {
    rottenTomatoes: "test_movie",
    metacritic: "test-movie",
    letterboxd: "test-movie",
    douban: "12345",
    allocineFilm: "12345",
  },
...
describe("runFetchers", () => {
  ...
  it("returns normalized scores and overall", async () => {
    ...
    const allocinePress = res.sources.find(
  ...
  it("persists KV-cached data to Postgres as backfill (insert-if-absent)", async () => {
    ...
    const kvPayload: ScorePayload = {
      movie: {
        imdbId: "tt-kv-persist",
        title: "KV Persist Movie",
        year: "2010",
      },
  ...
  it("persists in-memory cached data to Postgres as backfill (insert-if-absent)", async () => {
    const { persistScores } = await import("@/db/persist");
    const movie = {
      imdbId: "tt-mem-persist",
      title: "Mem Persist Movie",
      year: "2010",
    };
  ...
  it("writes to KV after scraping on KV miss", async () => {
    ...
    expect(mockKvSet).toHaveBeenCalledWith("tt-kvmiss", res, undefined, "2010");
  });
});
