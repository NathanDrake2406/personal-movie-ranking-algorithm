...
export async function resolveMovie(query: string, env: Record<string, string | undefined>, signal?: AbortSignal): Promise<ResolveResult> {
  ...
  const omdbResults = await fetchOmdbByTitleWithRotation(query, omdbKeys, signal);
