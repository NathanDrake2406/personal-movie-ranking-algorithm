...
const DEFAULT_PERSISTED_HASH = '33e1aa46f0f8a40bda4e54b9e3c9cb70a2c8c467ce7b01bb03e887c1641b3024';
...
function buildVariables(template: string, imdbId: string, themeId: string): Record<string, unknown> | null {
  const raw = template
...
function buildGraphqlPayload(imdbId: string, themeId: string, env: ImdbThemeEnv): Record<string, unknown> | null {
  const operationName = env.IMDB_THEME_GQL_OPERATION || DEFAULT_OPERATION_NAME;
  const query = env.IMDB_THEME_GQL_QUERY;
  const persistedHash = env.IMDB_THEME_GQL_PERSISTED_HASH || DEFAULT_PERSISTED_HASH;
  const variablesTemplate = env.IMDB_THEME_GQL_VARIABLES || DEFAULT_VARIABLES_TEMPLATE;
...
export async function fetchImdbThemeSummary(
  ...
  try {
    const hasQuery = typeof payload.query === 'string' && payload.query.length > 0;
    const hasPersisted = payload.extensions && typeof payload.extensions === 'object';

    const response = await (async () => {
      if (!hasQuery && hasPersisted) {
        const url = new URL(graphqlUrl);
        const operationName = (payload.operationName as string | undefined) || DEFAULT_OPERATION_NAME;
