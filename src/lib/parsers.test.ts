...
describe("parsers", () => {
  ...
  describe("parseImdbHtml", () => {
    it("extracts rating from aggregateRating JSON-LD", () => {
      const html =
        '"aggregateRating":{"ratingValue":8.6,"ratingCount":1234567}';
  ...
  describe("Douban ID extractors", () => {
    describe("parseDoubanSubjectSearchHtml", () => {
      ...
      it("returns null when no subject found", () => {
        expect(
          parseDoubanSubjectSearchHtml("<html>No results</html>"),
        ).toBeNull();
      });
    });
    ...
    describe("parseGoogleDoubanSearchHtml", () => {
      ...
      it("returns null when no Douban link found", () => {
        expect(
          parseGoogleDoubanSearchHtml("<html>No results</html>"),
        ).toBeNull();
      });
    });
  });
  ...
  describe("parseRTCriticsHtml", () => {
    it("extracts all critic scores", () => {
      const html =
        '"criticsAll":{"score":"92","averageRating":"8.1","ratingCount":245},"criticsTop":{"averageRating":"7.9","ratingCount":52}';
    ...
    it("handles missing top critics", () => {
      const html =
        '"criticsAll":{"score":"75","averageRating":"6.5","ratingCount":100}';
      const result = parseRTCriticsHtml(html);
  ...
  describe("parseRTAudienceHtml", () => {
    it("extracts verified audience score", () => {
      const html =
        '"audienceVerified":{"averageRating":"4.2","reviewCount":10000},"audienceAll":{"averageRating":"3.8"}';
  ...
  describe("parseImdbThemes", () => {
    it("extracts themes with positive sentiment", () => {
      ...
      expect(result).toEqual([
        {
          id: "authentic-emotion",
          label: "Authentic emotion",
          sentiment: "positive",
        },
        {
          id: "cinematography",
    ...
    it("uses __NEXT_DATA__ theme id to enrich aria-label chips", () => {
      const data = {
        props: {
          pageProps: {
            mainColumnData: {
              reviewSummary: {
                themes: [{ themeId: "theme-456", label: "Cinematography" }],
              },
            },
          },
        },
      ...
      const result = parseImdbThemes(html);
      expect(result).toEqual([
        { id: "theme-456", label: "Cinematography", sentiment: "positive" },
      ]);
    });
      ...
      const data = {
        data: {
          themeSummary: {
            id: "theme-123",
            plaidHtml:
              "Reviewers say the cinematography dazzles. AI-generated from user reviews",
          },
        },
      };
